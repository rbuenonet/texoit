...
const ListPage: React.FC = () => {
  ...
  const [titles, setTitles] = useState<Array<string | titleAdvanced>>([
    'Id',
    {
      value: '',
      text: 'Year',
      options: Array.from({ length: currentYear - 1970 + 1 }, (_, index) =>
  ...
  const changeFilter = (filter: string, value: string) => {
    value = filter === 'Winner' ? (value === 'yes' ? 'true' : 'false') : value
    setPage(1)
  ...
  const getFilter = (filter: string) => {
    const titleObject = titles.find(
      (item) => typeof item !== 'string' && item.text === filter
    )
  ...
  const fetchData = async () => {
    try {
      ...
      setContent(
        result.content.slice(0, pageSize).map((current: any) => ({
          id: current.id,
          year: current.year,
          title: current.title,
  ...
  return (
    <Container>
      ...
      {!content ||
        (content.length === 0 && <Table data={DefaulValue} titles={titles} />)}
    </Container>
  )
}
