...
const ListMovieWinnerByYear: React.FC<ListMovieWinnerByYearProps> = ({content, selectYear}) => {
  ...
  return (
    <Container>
        ...
        {newData && newData.length > 0 && <Table data={newData} titles={titles} />}
        {(!newData || newData.length === 0) && <Table data={DefaulValue} titles={titles} />}
    </Container>
  );
};
