...
import {TableContainer, TableRow, TableHeaderCell, TableCell, TablePage, TablePageCell, PageButton} from './style'
import {TableProps, titleAdvanced} from './props'

const Table: React.FC<TableProps> = ({data, titles, currentPage, totalPages = 1, onPageChange}) => {
                    ...
                    <TableHeaderCell key={column}>
                        ...
                        {titles && typeof titles[index] !== 'string' &&
                            <Selectbox 
                                ...
                                onSelect={(value) => (titles[index] as titleAdvanced).onChange(value)}
                            />
                        }
                    </TableHeaderCell>
                )}
            ...
            <tfoot>
                <TablePage>
                    <TablePageCell colSpan={columns.length}>
                        ...
                        <PageButton onClick={() => clickPage(1)} disabled={currentPage === 1}>
                        &lt;&lt;
                        </PageButton>
                        <PageButton onClick={() => clickPage(currentPage - 1)} disabled={currentPage === 1}>
                        &lt;
                        </PageButton>

                        {getPageNumbers(currentPage, totalPages).map(pageCurrent => 
                            <PageButton key={pageCurrent} onClick={() => clickPage(pageCurrent)} className={currentPage === pageCurrent ? 'active' : ''}>
                                {pageCurrent}
                            </PageButton>
                        )}

                        <PageButton onClick={() => clickPage(currentPage + 1)} disabled={currentPage === totalPages}>
                        &gt;
                        </PageButton>
                        <PageButton onClick={() => clickPage(totalPages ?? 1)} disabled={currentPage === totalPages}>
                        &gt;&gt;
                        </PageButton>
                    </TablePageCell>
                </TablePage>
